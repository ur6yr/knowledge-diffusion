...
class AgentRun:
    def __init__(self, store, profile: Provider, allow_mock: bool, seed_id: str):
        ...
        self.budget = current_budget(profile)
        self.started = time.monotonic()

    async def turn(self, role, function, arguments, sources=(), task=None):
        ...
        async def guarded(*args, **kwargs):
            if outputs:
                raise ValueError("Only one successful tool call per bounded role turn")
            started = now()
            task_id = digest([role, function.__name__, kwargs, sources, prompt_hash,
                              self.profile.model, f"kdiff-{__version__}"])
            try:
                ...
                receipt = ToolReceipt(run_id=self.run_id, seed_id=self.seed_id, instance_id="coordinated-worker",
        ...
        client = client_factory(self.profile, mock_call=(function.__name__, arguments), allow_mock=self.allow_mock,
                                budget=self.budget if self.profile.profile != 'mock' else None)
        ...
        try:
            result = await asyncio.wait_for(agent.run(task=task or ("Execute your scoped tool using this typed input: " +
            ...
            self.events.append({'role': role, 'event': {'type': 'RoleTiming', 'elapsed_seconds': time.monotonic() - role_started}})
            await client.close()
    ...
    def save(self, outcome):
        record = {"run_id": self.run_id, "seed_id": self.seed_id, "provider": self.profile.profile,
                  ...
                  'budget': self.budget.report(), 'elapsed_seconds': time.monotonic() - self.started}
